...
- If Current Stage is CONTACT_CAPTURE: STAY IN CONTACT_CAPTURE while contact info is being requested, provided, or validated. When the user sends their contact details, STAY in CONTACT_CAPTURE so the agent can validate them using tools. ONLY move to PLANS if the contact information has already been successfully verified and accepted by the agent, OR if the user explicitly refuses to provide it (e.g., "no", "not comfortable", "just tell me here").
