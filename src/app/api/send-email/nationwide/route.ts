...
/**
 * Generates a luxury, high-converting HTML email template for the Nationwide Publishing plan.
 */
function generateExecutiveProposalHtml(clientName: string, refId?: string, price: string = '$999'): string {
  ...
  const content = `
    ...
    ${renderPlanSection('Nationwide Publishing Plan', price, nationwideFeatures, 'nationwide')}
    ${renderAssurancesSection()}
    ${renderCallToAction()}
  `;
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Nationwide executive proposal email sent successfully!'
    });
